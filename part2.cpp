...
void processExpansion(char *filename)
{
    ...
    const long fsize = filestatus.st_size;
    ...
    readIn.close();
    // logger << "Saved String: " << s << "\n";
    std::vector<int> blocks;
    for (int i = 0; i < s.size(); i += bits)
    {
        std::string toConvert = s.substr(i, bits);
        // logger << "Processing with: " << bits << "bits, " << binaryString2Int(toConvert) << '\n';
        if (isOnly(toConvert, '1')) {
            std::cout << "Switching to " << bits + 1 << " bits? " << (bits < 16 ? "Yes" : "no") << '\n';
            bits += (bits < 16 ? 1 : 0);
        }
        logger << "Bits: " << bits << '\n';
        blocks.push_back(binaryString2Int(s.substr(i, bits)));
    }
    std::cout << blocks.size() << '\n';
    std::string decomopressed = decompress(blocks.begin(), blocks.end());
    logger << "Decompressed String: " << decomopressed << '\n';
}
